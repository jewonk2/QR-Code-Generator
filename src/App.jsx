...
const PINK = { bg: "#FFEAF3", grid: "#FBD0E4", surface: "#FFF6FB", ink: "#C85088", sub: "#DD93BC", line: "#F3BCD8", accent: "#EC4899" };
...
export default function App() {
  ...
  const [centerBold, setCenterBold] = useState(false);
  const [centerItalic, setCenterItalic] = useState(false);
  const [centerUnderline, setCenterUnderline] = useState(false);
  const [centerFont, setCenterFont] = useState("sans-serif");
  const [centerTextColor, setCenterTextColor] = useState("#16223A");
  const [centerImgURL, setCenterImgURL] = useState(null);  // SVG 임베드용 dataURL
  const [centerImgName, setCenterImgName] = useState("");  // 선택한 이미지 파일명
  ...
  const fmtBtn = (on) => ({ width: 34, height: 30, borderRadius: 7, border: `1px solid ${on ? TH.accent : TH.line}`, background: on ? TH.accent : "#fff", color: on ? "#fff" : TH.ink, cursor: "pointer", fontSize: 15, lineHeight: 1, display: "inline-flex", alignItems: "center", justifyContent: "center" });
  ...
  useEffect(() => {
    ...
    const SS = 3;                              // 슈퍼샘플링 배율: 고해상도로 그린 뒤 축소 표시 → 텍스트/이미지 선명
    cv.width = total * SS; cv.height = total * SS;
    ...
    // 가운데 삽입: 정확히 n×n 정중앙 셀을 비우고 이미지/텍스트 배치 (애니메이션 재생 중에는 숨김)
    if (centerMode !== "none" && !playing) {
      ...
      try {
        if (centerMode === "image" && imgBmpRef.current) {
          ...
          ctx.fillStyle = centerTextColor; ctx.textAlign = "center"; ctx.textBaseline = "middle";
          const fstyle = `${centerItalic ? "italic " : ""}${centerBold ? "800" : "500"}`;
          let fs = wpx; ctx.font = `${fstyle} ${fs}px ${centerFont}`;
          while (fs > 6 && ctx.measureText(centerText).width > wpx * 0.74) { fs -= 1; ctx.font = `${fstyle} ${fs}px ${centerFont}`; }  // 기본 크기 축소(0.74)
          const cx = x + wpx / 2, cy = y + wpx / 2;
          ctx.fillText(centerText, cx, cy);
          if (centerUnderline) { const tw = ctx.measureText(centerText).width, ly = cy + fs * 0.42; ctx.strokeStyle = centerTextColor; ctx.lineWidth = Math.max(1, fs * 0.07); ctx.beginPath(); ctx.moveTo(cx - tw / 2, ly); ctx.lineTo(cx + tw / 2, ly); ctx.stroke(); }
          ctx.textAlign = "left"; ctx.textBaseline = "alphabetic";
        }
      } catch (e) { /* 이미지/텍스트 그리기 실패해도 렌더·애니메이션이 멈추지 않도록 무시 */ }
    }
  }, [result, viewMode, quiet, playing, displayMatrix, centerMode, centerText, centerImgURL, imgTick, centerN, centerBold, centerItalic, centerUnderline, centerFont, centerTextColor]);
  ...
  // 웹폰트가 로드되면 캔버스를 한 번 다시 그려 선택한 글꼴이 PNG에도 반영되게
  useEffect(() => { if (typeof document !== "undefined" && document.fonts && document.fonts.ready) document.fonts.ready.then(() => setImgTick(t => t + 1)); }, []);
  ...
  const downloadSVG = () => {
    ...
    if (centerMode !== "none") {
      ...
      else if (centerMode === "text" && centerText) { const esc = centerText.replace(/[<>&]/g, m => ({ "<": "&lt;", ">": "&gt;", "&": "&amp;" }[m])); const deco = centerUnderline ? ` text-decoration="underline"` : ""; extra += `<text x="${cs + b + cN / 2}" y="${cs + b + cN / 2}" font-size="${cN * 0.5}" font-family="${centerFont}" font-weight="${centerBold ? 800 : 500}" font-style="${centerItalic ? "italic" : "normal"}" fill="${centerTextColor}"${deco} text-anchor="middle" dominant-baseline="central">${esc}</text>`; }
    }
  ...
  return (
    <div style={{ minHeight: "100vh", position: "relative", overflow: "hidden", background: romantic ? "linear-gradient(160deg,#FFEAF3,#FFD9EC 60%,#FFE8F4)" : TH.bg, backgroundImage: romantic ? "none" : `linear-gradient(${TH.grid} 1px,transparent 1px),linear-gradient(90deg,${TH.grid} 1px,transparent 1px)`, backgroundSize: "22px 22px", color: TH.ink, fontFamily: "ui-sans-serif,system-ui,'Segoe UI',sans-serif", padding: "32px 18px 64px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1.1fr)", gap: 20, alignItems: "start" }} className="qr-grid">
          ...
          <section style={{ background: TH.surface, border: `1px solid ${TH.line}`, borderRadius: 14, padding: 20, position: "sticky", top: 20 }}>
            <div style={{ display: "flex", justifyContent: "center", alignItems: "center", background: viewMode !== "plain" ? "#FBFBFD" : "#fff", borderRadius: 10, padding: 12, minHeight: 200 }}>
              {result.error
                ...
                : <canvas ref={canvasRef} style={{ width: "100%", maxWidth: 360, height: "auto", imageRendering: "auto" }} />}
            </div>
            {!result.error && (
              <>
          ...
          <section style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={card()}>
              ...
              <textarea value={url} onChange={e => setUrl(e.target.value)} placeholder="https://... 또는 여러 줄 텍스트" rows={3} style={{ ...input(), fontFamily: "var(--qr-font)", minHeight: 78, resize: "vertical", lineHeight: 1.5 }} />
            ...
            {!result.error && (() => {
              ...
              return (
                <div style={card()}>
                  <label style={lbl()}>요소 삽입 (이미지 / 텍스트)</label>
                    ...
                    <div style={{ marginBottom: 10 }}>
                      <div onDrop={onDropImage} onDragOver={e => e.preventDefault()} onPaste={onPasteImage} tabIndex={0}
                        ...
                        <div style={{ marginTop: 8 }}>
                          <label style={{ display: "inline-block", padding: "8px 14px", borderRadius: 8, border: `1px solid ${TH.line}`, background: "#fff", color: TH.ink, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                            {centerImgURL ? "다른 이미지 선택" : "이미지 선택"}
                      ...
                      {centerImgURL && (
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8, padding: 8, border: `1px solid ${TH.line}`, borderRadius: 8, background: "#fff" }}>
                          <img src={centerImgURL} alt="" style={{ width: 34, height: 34, objectFit: "contain", borderRadius: 5, border: `1px solid ${TH.line}`, background: "#fff" }} />
                          <span style={{ fontSize: 12, color: TH.ink, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>{centerImgName || "선택한 이미지"}</span>
                          <button onClick={() => { setCenterImgURL(null); setCenterImgName(""); imgBmpRef.current = null; setImgTick(t => t + 1); }} style={{ border: "none", background: "transparent", color: TH.sub, cursor: "pointer", fontSize: 16, lineHeight: 1 }} title="제거">×</button>
                        </div>
                      )}
                    </div>
                  )}
                  {centerMode === "text" && (
                    <div style={{ marginBottom: 10 }}>
                      <input value={centerText} onChange={e => setCenterText(e.target.value)} maxLength={6} placeholder="" style={{ ...input(), marginBottom: 8, fontFamily: centerFont }} />
                      <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 6 }}>
                        <select value={centerFont} onChange={e => setCenterFont(e.target.value)} style={{ ...input(), width: "auto", flex: "0 0 auto", padding: "6px 8px", fontFamily: centerFont }}>
                          <option value="sans-serif" style={{ fontFamily: "sans-serif" }}>고딕</option>
                          <option value="serif" style={{ fontFamily: "serif" }}>바탕</option>
                          <option value="'Black Han Sans', sans-serif" style={{ fontFamily: "'Black Han Sans', sans-serif" }}>검은고딕</option>
                          <option value="'Jua', sans-serif" style={{ fontFamily: "'Jua', sans-serif" }}>둥근고딕</option>
                          <option value="'Nanum Pen Script', cursive" style={{ fontFamily: "'Nanum Pen Script', cursive" }}>손글씨</option>
                        </select>
                        <button onClick={() => setCenterBold(v => !v)} title="굵게" style={{ ...fmtBtn(centerBold), fontWeight: 800 }}>가</button>
                        <button onClick={() => setCenterItalic(v => !v)} title="기울임" style={{ ...fmtBtn(centerItalic), fontStyle: "italic" }}>가</button>
                        <button onClick={() => setCenterUnderline(v => !v)} title="밑줄" style={{ ...fmtBtn(centerUnderline), textDecoration: "underline" }}>가</button>
                        <label title="글자색" style={{ position: "relative", display: "inline-flex", alignItems: "center", gap: 4, height: 30, padding: "0 7px", borderRadius: 7, border: `1px solid ${TH.line}`, background: "#fff", cursor: "pointer" }}>
                          <span style={{ display: "inline-flex", flexDirection: "column", alignItems: "center", lineHeight: 1 }}>
                            <span style={{ fontSize: 13, fontWeight: 700, color: TH.ink }}>가</span>
                            <span style={{ width: 16, height: 3, borderRadius: 2, marginTop: 1, background: centerTextColor }} />
                          </span>
                          <span style={{ fontSize: 8, color: TH.sub }}>▾</span>
                          <input type="color" value={centerTextColor} onChange={e => setCenterTextColor(e.target.value)} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: 0, cursor: "pointer" }} />
                        </label>
                      </div>
                    </div>
                  )}
      ...
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Noto+Sans:wght@400;500;700&family=Noto+Sans+KR:wght@400;500;700&family=Black+Han+Sans&family=Jua&family=Nanum+Pen+Script&display=swap');
        /* 입력란 글꼴: 아래 한 줄만 바꾸면 됨.
           - 다른 구글 폰트: 위 @import의 family=... 와 아래 이름을 함께 교체
           - 내 폰트 파일: 위 @import 대신 @font-face{ font-family:'MyFont'; src:url('/fonts/MyFont.woff2') format('woff2'); } 추가 후 아래를 'MyFont'로 */
        :root{ --qr-font: 'Noto Sans', 'Noto Sans KR', ui-sans-serif, system-ui, sans-serif; }
