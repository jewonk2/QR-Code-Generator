...
function generateQR(text, ec = "M") {
  ...
  return {
    matrix: best, baseMatrix: matrix, typeGrid: matrix.tp, blockGrid, kindGrid, cwIndexGrid, origin, tape, nBlocks, version, ec, mask: bestMask, size: matrix.size,
...
const TYPE_LABEL = {
  ...
  ec: ["오류정정(EC)", "Reed-Solomon 복원용 코드워드"],
};
...
function Swatch({ color, label, hatch }) {
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12, color: C.sub }}>
      <span style={{ width: 11, height: 11, background: "#fff", backgroundImage: hatch ? `repeating-linear-gradient(45deg, ${color} 0 1.5px, #fff 1.5px 4px)` : "none", backgroundColor: hatch ? "#fff" : color, border: hatch ? `1px solid ${C.line}` : "none", boxSizing: "border-box", borderRadius: 2, flexShrink: 0 }} />{label}
    </span>
  );
}
...
export default function App() {
  ...
  const [centerText, setCenterText] = useState("Q");
  const [centerImg, setCenterImg] = useState(null);     // HTMLImageElement
  ...
  const onPickImage = (e) => {
    ...
    rd.onload = () => { const im = new Image(); im.onload = () => { setCenterImg(im); setCenterMode("image"); }; im.src = rd.result; };
    rd.readAsDataURL(f);
  };
  ...
  useEffect(() => {
    ...
    // 가운데 삽입: n×n 정중앙 셀을 비우고 이미지/텍스트 배치
    if (centerMode !== "none") {
      const cN = Math.max(1, Math.min(centerN, size - 8));
      const cs = Math.round((size - cN) / 2);
      const x = (cs + BORDER) * PX, y = (cs + BORDER) * PX, wpx = cN * PX, pad = Math.max(2, PX * 0.35);
      ctx.fillStyle = "#FFFFFF"; ctx.fillRect(x - pad, y - pad, wpx + pad * 2, wpx + pad * 2);
      if (centerMode === "image" && centerImg) {
        const iw = centerImg.naturalWidth || centerImg.width, ih = centerImg.naturalHeight || centerImg.height;
        const sc = Math.min(wpx / iw, wpx / ih), dw = iw * sc, dh = ih * sc;
        ctx.drawImage(centerImg, x + (wpx - dw) / 2, y + (wpx - dh) / 2, dw, dh);
        ...
        while (fs > 6 && ctx.measureText(centerText).width > wpx) { fs -= 1; ctx.font = `800 ${fs}px ui-sans-serif,system-ui,sans-serif`; }
        ctx.fillText(centerText, x + wpx / 2, y + wpx / 2);
        ctx.textAlign = "left"; ctx.textBaseline = "alphabetic";
      }
    }
  }, [result, viewMode, quiet, playing, displayMatrix, centerMode, centerText, centerImg, centerN]);
  ...
  useEffect(() => {
    ...
    const ctx = cv.getContext("2d");
    const { size, typeGrid, origin, tape, nBlocks, cwIndexGrid } = result; const n = origin.length;
    ...
    // 클러스터(코드워드 = 2×4 모듈) 레이아웃 — 화면에 맞게 모듈 크기 자동 축소
    const topY = PX * 3.2;
    let mc = Math.max(3, Math.round(PX * 0.46)), clusterW, clusterH, gap, CPR;
    const layout = (m) => { clusterW = 2 * m; clusterH = 4 * m; gap = Math.max(2, Math.round(m * 0.7)); CPR = Math.max(4, Math.floor((total * 0.94) / (clusterW + gap))); return topY + Math.ceil(n / CPR) * (clusterH + gap); };
    while (layout(mc) > total * 0.96 && mc > 2) mc--; layout(mc);
    const offX = (total - (CPR * (clusterW + gap) - gap)) / 2;
    const clusterCenter = (ci) => ({ x: offX + (ci % CPR) * (clusterW + gap) + clusterW / 2, y: topY + Math.floor(ci / CPR) * (clusterH + gap) + clusterH / 2 });
    ...
    const totalMsgBytes = bo, totalDataCw = result.stats.dataCw;
    ...
    for (let j = 0; j < totalDataCw; j++) { const key = ownerOf(j); if (!cur || cur.key !== key) { if (cur) segs.push(cur); const isC = key[0] === "c"; cur = { key, start: j, end: j + 1, isChar: isC, glyph: isC ? chars[+key.slice(1)] : (key === "h" ? "헤더" : "패딩") }; } else cur.end = j + 1; }
    if (cur) segs.push(cur);
    if (n > totalDataCw) segs.push({ key: "ec", start: totalDataCw, end: n, isChar: false, glyph: "EC" });
    const nChar = segs.filter(s => s.isChar).length || 1, kk = Math.min(1, 20 / nChar);
    let acc = 0; const genByTi = new Array(n);
    segs.forEach(s => { s.dur = Math.max(s.isChar ? 90 : 80, (s.isChar ? 300 : s.key === "ec" ? 440 : 240) * kk); s.t0 = acc; acc += s.dur; const span = s.end - s.start; let sx = 0, sy = 0; for (let ti = s.start; ti < s.end; ti++) { genByTi[ti] = s.t0 + ((ti - s.start) / span) * s.dur * 0.72; const cc = clusterCenter(ti); sx += cc.x; sy += cc.y; } s.cx = sx / span; s.cy = sy / span; });
    const P1 = acc, SPLIT = 950, PLACE = 2800, P2 = P1 + SPLIT, ENDT = P2 + PLACE;
    ...
    const frame = (now) => {
      ...
      for (const p of parts) {
        let cx, cy, s;
        if (T < P1) {                                   // ① 생성: 문자 순서대로 모듈 묶음 등장
          if (T < p.gen) continue; const c0 = clusterCenter(p.ti); cx = c0.x + p.off.dx; cy = c0.y + p.off.dy; s = mc;
        } else if (T < P2) {                            // ② 분할: 블록 간격 벌어짐
          const e = ease((T - P1) / SPLIT); const c0 = clusterCenter(p.ti), c1 = clusterCenter(p.ti + p.b * 2);
          ...
          const c1 = clusterCenter(p.ti + p.b * 2), sx = c1.x + p.off.dx, sy = c1.y + p.off.dy;
          cx = sx + (p.rx - sx) * e; cy = sy + (p.ry - sy) * e; s = mc + (PX - mc) * e;
        }
        ctx.fillStyle = cellCol(p.b, p.kd, p.on); ctx.fillRect(cx - s / 2, cy - s / 2, s, s);
      }

      // 문자/구간 라벨: 해당 셀 묶음이 생성되는 동안 위에 떴다가 천천히 사라짐
      ctx.textAlign = "center"; ctx.textBaseline = "alphabetic";
      for (const s of segs) {
        const win = s.dur * 1.4;                 // 완전히 사라지는 시점을 살짝 늦춤
        const lt = T - s.t0; if (lt < 0 || lt > win) continue;
        let a = 1;
        if (lt < 110) a = lt / 110;
        else if (lt > win * 0.72) a = Math.max(0, 1 - (lt - win * 0.72) / (win * 0.28));
        ctx.globalAlpha = a;
        if (s.isChar) { ctx.fillStyle = C.accent; ctx.font = `700 ${Math.round(Math.max(14, mc * 3.4))}px ui-sans-serif,system-ui,sans-serif`; }
        else { ctx.fillStyle = C.sub; ctx.font = `600 ${Math.round(Math.max(11, mc * 2.2))}px ui-sans-serif,system-ui,sans-serif`; }
  ...
  const downloadSVG = () => {
    ...
    let extra = "";
    if (centerMode !== "none") {
      const cN = Math.max(1, Math.min(centerN, size - 8)), cs = Math.round((size - cN) / 2), pad = 0.4;
      extra += `<rect x="${cs + b - pad}" y="${cs + b - pad}" width="${cN + pad * 2}" height="${cN + pad * 2}" fill="#fff"/>`;
      if (centerMode === "image" && centerImg) extra += `<image href="${centerImg.src}" x="${cs + b}" y="${cs + b}" width="${cN}" height="${cN}" preserveAspectRatio="xMidYMid meet"/>`;
              ...
              <>
                ...
                {viewMode === "blocks" && (
                  <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px dashed ${C.line}` }}>
                    ...
                    <div style={{ fontSize: 11.5, color: C.sub, marginBottom: 5 }}>인터리빙 전 — 블록별로 뭉쳐 있는 코드워드 띠</div>
                    <div style={{ display: "flex", height: 18, borderRadius: 5, overflow: "hidden", border: `1px solid ${C.line}` }}>
                      {(() => {
                        ...
                        blocksInfo.forEach((b, i) => segs.push(<div key={"e" + i} title={`블록 ${i + 1} EC ${b.ecLen}개`} style={{ flexGrow: b.ecLen, background: shade(BLOCK_PALETTE[i % L], 0.32) }} />));
                        return segs;
                      })()}
                    </div>
                    <div style={{ fontSize: 11, color: C.sub, marginTop: 4 }}>왼쪽=데이터 코드워드 전체 · 오른쪽(진한색)=오류정정 전체. 애니메이션은 빈 매트릭스 위에서 이 코드워드들이 블록으로 쪼개져 격자 곳곳에 흩어져 채워지는 과정을 보여줍니다.</div>
                ...
                {viewMode === "masks" && (
                  <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px dashed ${C.line}` }}>
                    <div style={{ fontSize: 12, color: C.sub, marginBottom: 8, lineHeight: 1.5 }}>
                      마스크 8종의 페널티 점수를 계산해 <b style={{ color: C.ink }}>가장 낮은 #{result.mask}</b>을 자동 선택합니다. 점수가 낮을수록 명암이 고르게 퍼져 잘 읽힙니다. 썸네일을 누르면 다른 마스크를 직접 적용해 비교할 수 있습니다.
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 7 }}>
                      {result.stats.scores.map((sc, mk) => (
                        <div key={mk} onClick={() => setMaskOverride(mk)} title={`마스크 #${mk} 적용`}
                          style={{ textAlign: "center", padding: 6, borderRadius: 8, cursor: "pointer", border: `2px solid ${mk === activeMask ? C.accent : C.line}`, background: mk === activeMask ? "#EEF2FF" : "#fff" }}>
                          <MaskThumb result={result} mk={mk} />
                          <div style={{ fontSize: 11, fontWeight: 700, marginTop: 4 }}>#{mk}{mk === result.mask ? " ★" : ""}{mk === activeMask && mk !== result.mask ? " ✓" : ""}</div>
                          <div style={{ fontSize: 10.5, color: mk === activeMask ? C.accent : C.sub }}>{sc}점</div>
                        </div>
                      ))}
                    </div>
                    <div style={{ fontSize: 11, color: C.sub, marginTop: 8 }}>★ 자동 선택(최저 점수) · ✓ 현재 수동 선택</div>
                    {maskOverride != null && maskOverride !== result.mask && (
                      <button onClick={() => setMaskOverride(null)} style={{ ...btn(false), marginTop: 10 }}>자동 선택(#{result.mask})으로 되돌리기</button>
                    )}
                  </div>
                )}
              </>
            ...
            {!result.error && (
              <div style={card}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                  ...
                  <Stat label="적용된 마스크" value={`#${activeMask}`} sub={maskOverride != null && maskOverride !== result.mask ? `수동 · 점수 ${result.stats.scores[activeMask]}` : `자동 · 점수 ${result.stats.bestScore}`} />
